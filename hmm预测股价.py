# -*- coding: utf-8 -*-
"""
Created on Sun Apr 24 00:27:41 2022
...
#数据集的划分
X_Test = X.iloc[0:] #[:-30]倒数30index前的作训练集
X_Pre = X.iloc[-31:]#倒数30index作测试集
...
expected_returnss =  expected_returns_volumes[:,1]
predicted_price = []  #预测值
#predicted_volume = []#预测成交量
current_price = close_v[325]
#current_volume = volume[1293]


X_Pre.iloc[0]=X_Pre.iloc[10]
for i in range(1,11):
    X_Pre.iloc[i]=0
...
for i in range(10):
   ...
   #predicted_volume.append(expected_returnss[hidden_states])
   current_price = predicted_price[i]
   #current_volume = predicted_volume[i]
   #X_Pre.iloc[i+1][0]=current_price
   #X_Pre.iloc[i+1][1]=predicted_volume[i]
